...
void get_file(char* username, char* password, char* salt, char* to_get) {
    ...
    uint32_t amount_received = 1;
    uint32_t length_of_last; //save the length of the last block to adjust 'datasize'.
    // Loop if the block extracted is not the last one.
    while (amount_received != ServerResponse->header.blockCount) {
        if (ServerResponse->header.blockNum+1 == ServerResponse->header.blockCount) {
            length_of_last = ServerResponse->header.responseLen;
        }
    ...
    datasize = datasize - ((MAX_PAYLOAD)-(length_of_last));
    free(ServerResponse);
    printf("datasize: %li\n", datasize);
